...
def main():
    ...
    for gamma in range(2, 3):
        for f1 in os.listdir(path):
            ...
            fileResultTime = '../../Results/' + base + '/ResultsFilesTimeG3_10.csv'
            ...
            resultsTallLS = collections.defaultdict(int)
            resultsTallDEC = collections.defaultdict(int)
            resultsTallLSAR = collections.defaultdict(int)
            resultsTallBBR19 = collections.defaultdict(int)
            ...
            if os.path.isdir(pathF1):
                    ...
                    if os.path.isdir(pathF2):
                        ...
                        for file in os.listdir(pathF2):
                            ...
                            if file.endswith('.linkstream'):
                                ...
                                dataTocsvLS = list(map(lambda x: [x[0], len(x[1])], list(resultsTLS.items())))
                                for e in dataTocsvLS:
                                    resultsTallLS[e[0]] += e[1]
                                ...
                                resultsTDEC = collections.defaultdict(list)
                                for e in M:
                                    resultsTDEC[e[0]].append(e)
                                dataTocsvDEC = list(map(lambda x: [x[0], len(x[1])], list(resultsTDEC.items())))

                                for e in dataTocsvDEC:
                                    resultsTallDEC[e[0]] += e[1]
                                ...
                                dataTocsvBBR19 = list(map(lambda x: [x[0], len(x[1])], list(resultsTBBR19.items())))

                                for e in dataTocsvBBR19:
                                    resultsTallBBR19[e[0]] += e[1]
                                ...
                                nb_gmLSAR, resultsTLSAR = g_m_n.gammaMatching(g_edges, gamma)
                                timeLSAR = round(time.time() - start_time, 4)
                                dataTocsvLSAR = list(map(lambda x: [x[0], len(x[1])], list(resultsTLSAR.items())))

                                for e in dataTocsvLSAR:
                                    resultsTallLSAR[e[0]] += e[1]
                ...
                for e in resultsTallLS.items():
                    resultsTallLS[e[0]] = e[1] / nb_file

                dataToCsv('../../Results/' + base + '/LS', resultsTallLS)

                for e in resultsTallDEC.items():
                    resultsTallDEC[e[0]] = e[1] / nb_file
                dataToCsv('../../Results/' + base + '/DEC', resultsTallDEC)

                for e in resultsTallLSAR.items():
                    resultsTallLSAR[e[0]] = e[1] / nb_file
                dataToCsv('../../Results/' + base + '/LSAR', resultsTallLSAR)

                for e in resultsTallBBR19.items():
                    resultsTallBBR19[e[0]] = e[1] / nb_file
                dataToCsv('../../Results/' + base + '/BBR19', resultsTallBBR19)
